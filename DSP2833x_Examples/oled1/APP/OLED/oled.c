...
//延时函数
void  delay_ms1(unsigned int nDelay)
{
    int ii,jj,kk=0;
...
void OLED_Init(void)
{
    EALLOW;
       ...
       GpioCtrlRegs.GPAMUX1.bit.GPIO5=0;//'0'为数字I/O,'1'为外设I/O
       GpioCtrlRegs.GPADIR.bit.GPIO5=1;//'0'为输入,'1'为输出
       GpioCtrlRegs.GPAPUD.bit.GPIO5=1;//'0'为上拉，'1'为禁止上拉
...
*/ 
/*************************************************************************/
void OLED_ShowGBK(u8 x, u8 y,  u8 chr, u8 size,u8 mode)
{
    u8 temp,t,t1;
    u8 y0=y;
    //u8 size = 16;
    ...
    {  
        ...
        if(size==12)      temp = asc2_1206[chr][t];    //调用1212字体
        else if(size==16) temp = asc2_1608[chr][t];    //调用1616字体
        else if(size==24) temp = asc2_2412[chr][t];    //调用2412字体
        else return;                                //没有的字库
        for(t1=0;t1<8;t1++)
        {
