...
void EXTI1_Init(void)
{
	...
	GpioCtrlRegs.GPAQSEL1.bit.GPIO12 = 2;        // 外部中断1（XINT2）输入限定6个采样窗口
...
void EXTI3_Init(void)
{
	...
	//GpioCtrlRegs.GPAQSEL1.bit.GPIO14 = 0;        // 外部中断3（XINT1）与系统时钟SYSCLKOUT同步
	GpioCtrlRegs.GPAQSEL1.bit.GPIO14 = 2;        // 外部中断2（XINT2）输入限定6个采样窗口
	GpioCtrlRegs.GPACTRL.bit.QUALPRD1 = 0xFF;   // 每个采样窗口的周期为510*SYSCLKOUT
	...
	EALLOW;
	GpioIntRegs.GPIOXINT3SEL.bit.GPIOSEL = 34;   // XINT3是GPIO34(XINT3要求用GPIO32-GPIO63,所以此键盘电路不行,使用跳线连接GPIO14和GPIO34实现功能)
	EDIS;
